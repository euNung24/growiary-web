...
const ReportTotal = ({ month }: ReportTotal) => {
    ...
    <section>
      ...
      >
        {data && (
          <>
            <li>
              총 <Chip className={chipStyle}>{data.post?.user[month]}개</Chip>의 글을{' '}
              <Chip className={chipStyle}>{data.charCount?.[month].sum}자</Chip>로
              작성했어요.
            </li>
            <li>
              주로{' '}
              <Chip className={chipStyle}>
                {data.week?.[month] &&
                  DATE[data.week[month].toSorted((a, b) => (a > b ? -1 : 1))[0]]}
                요일
              </Chip>
              ,{' '}
              <Chip className={chipStyle}>
                {data.time?.[month] &&
                  TIME[data.time[month].toSorted((a, b) => (a > b ? -1 : 1))[0]]}
                시간
              </Chip>
              에 글 작성했어요.
            </li>
        ...
        {data && topNewTag && (
          <li>
            지난 {month !== 0 ? month : 12}월과 비교해 새로 등장한 태그는{' '}
