...
const buttonVariants = cva(
  "inline-flex items-center justify-center whitespace-nowrap rounded-md ring-offset-white transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-950 focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 dark:ring-offset-slate-950 dark:focus-visible:ring-slate-300",
  {
    variants: {
      variant: {
        default: "bg-primary-900 text-white-0 focus:bg-primary-900/90 dark:bg-primary-900 dark:text-white-0 dark:focus:bg-primary-900/90",
        secondary:
        ...
        ghost: "text-primary-900 hover:text-primary-900/90 focus:bg-primary-50 dark:hover:text-primary-900/90 dark:focus:bg-primary-50",
        ghostGray: "text-gray-900 hover:text-gray-900/90 focus:bg-gray-50 dark:hover:text-gray-900/90 dark:focus:bg-gray-50",
        destructive:
          "bg-red-500 text-slate-50 hover:bg-red-500/90 dark:bg-red-900 dark:text-slate-50 dark:hover:bg-red-900/90",
        link: "text-slate-900 underline-offset-4 hover:underline dark:text-slate-50",
      },
      size: {
        default: "h-10 px-3 py-[11px] font-sb14",
        sm: "h-8 px-4 py-2 font-sb12",
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, disabled = false, ...props }, ref) => {
    ...
    let disableClass = "pointer-events-none ";

    if (disabled) {
      switch (variant) {
        case 'outline':
          disableClass += 'border-gray-100 text-gray-400 dark:border-gray-100 dark:text-gray-400';
          break;
        case 'outlineGray':
          disableClass += 'border-gray-100 text-gray-400 dark:border-gray-100 dark:text-gray-400';
          break;
